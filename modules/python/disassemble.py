...
def disassemble_backwards(arch: gdb.Architecture, end_pc: int, offset: int, count: int):
    ...
    while len(disassembled) < (offset + 1):
        ...
        if block is None:
            disassembled = [
                {"addr": 0, "asm": "unknown"}
                for x in range(0, (offset + 1) - len(disassembled))
            ] + disassembled
        else:
            disassembled = (
                arch.disassemble(start_pc=block.start, end_pc=end_pc) + disassembled
            )
...
class DisassembleRequest(gdb.Command):
    ...
    def invoke(self, args, from_tty):
        [
            ...
            instructionCount,
            resolveSymbols,
        ] = midas_utils.parse_command_args(args, str, int, int, int, bool)
        ...
        midas_utils.send_response(
            self.name, {"instructions": result}, midas_utils.prepare_command_response
        )
